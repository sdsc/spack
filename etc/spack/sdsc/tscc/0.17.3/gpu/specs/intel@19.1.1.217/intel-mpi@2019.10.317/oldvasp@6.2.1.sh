...
declare -xr SPACK_PACKAGE='vasp@6.2.1'
declare -xr SPACK_COMPILER="intel@19.1.1.217"
declare -xr SPACK_VARIANTS="ldflags='-L/cm/local/apps/cuda/libs/current/lib64' +cuda ~openmp +scalapack ~vaspsol"
declare -xr SPACK_DEPENDENCIES="^intel-mpi@2019.10.317/$(spack find --format '{hash:7}' intel-mpi@2019.10.317 % ${SPACK_COMPILER}) ^intel-mkl@2020.4.304 % ${SPACK_COMPILER} ^cuda@11.2.2/$(spack find --format '{hash:7}' cuda@11.2.2 % ${SPACK_COMPILER})"
...
spack spec --long --namespaces --types vasp@6.2.1 % intel@19.1.1.217 +cuda ~openmp +scalapack ~vaspsol "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install -v --jobs 1 --fail-fast --yes-to-all vasp@6.2.1 % intel@19.1.1.217 +cuda ~openmp +scalapack  ~vaspsol "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
