...
spack spec --long --namespaces --types vasp@6.4.0 % intel@19.1.1.217 +cuda ~openmp +scalapack ~vaspsol "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install -v --jobs 1 --fail-fast --yes-to-all vasp@6.4.0 % intel@19.1.1.217 +cuda ~openmp +scalapack  ~vaspsol "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
