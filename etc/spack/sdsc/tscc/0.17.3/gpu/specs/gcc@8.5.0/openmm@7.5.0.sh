...
spack spec --long --namespaces --types openmm@7.5.0 % gcc@8.5.0 +cuda cuda_arch=60,80 ~ipo "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all openmm@7.5.0 % gcc@8.5.0 +cuda cuda_arch=60,80 ~ipo "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
