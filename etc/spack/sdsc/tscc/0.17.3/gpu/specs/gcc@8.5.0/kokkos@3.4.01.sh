...
spack spec --long --namespaces --types kokkos@3.4.01 % gcc@8.5.0 ~aggressive_vectorization ~compiler_warnings +cuda cuda_arch=60 +cuda_constexpr +cuda_lambda +cuda_ldg_intrinsic ~cuda_relocatable_device_code ~cuda_uvm ~debug ~debug_bounds_check ~debug_dualview_modify_check ~deprecated_code ~examples ~explicit_instantiation ~hpx ~hpx_async_dispatch ~hwloc ~ipo ~memkind ~numactl ~openmp +pic +profiling ~profiling_load_print ~pthread ~qthread ~rocm +serial +shared ~sycl ~tests ~tuning +wrapper "^cuda@11.2.2/$(spack find --format '{hash:7}' cuda@11.2.2 % ${SPACK_COMPILER}) ^kokkos-nvcc-wrapper ~mpi"
if [[ "${?}" -ne 0 ]]; then
