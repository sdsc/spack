...
declare -xr MPI_MODULE='openmpi@4.3.1'
...
declare -xr SPACK_VARIANTS="+cuda ~openmp +scalapack ~vaspsol"
declare -xr SPACK_DEPENDENCIES="^openmpi@4.1.3/$(spack find --format '{hash:7}' openmpi@4.1.3 % ${SPACK_COMPILER}) ^intel-mkl@2020.4.304 % ${SPACK_COMPILER} ^cuda@11.2.2/$(spack find --format '{hash:7}' cuda@11.2.2 % ${SPACK_COMPILER})"
...
spack spec --long --namespaces --types vasp@6.2.1 % gcc@10.2.0 +cuda ~openmp +scalapack ~vaspsol "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install -v --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all vasp@6.2.1 % gcc@10.2.0 +cuda ~openmp +scalapack ~vaspsol "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
