...
spack spec --long --namespaces --types relion@3.1.4 % gcc@10.2.0 +allow_ctf_in_sagd +cuda cuda_arch=60 +double ~double-gpu ~gui ~ipo ~mklfft "^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} +mpi ~openmp ^openmpi@4.1.3)"
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all relion@3.1.4 % gcc@10.2.0 +allow_ctf_in_sagd +cuda cuda_arch=60 +double ~double-gpu ~gui ~ipo ~mklfft "^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} +mpi ~openmp ^openmpi@4.1.3)"
if [[ "${?}" -ne 0 ]]; then
