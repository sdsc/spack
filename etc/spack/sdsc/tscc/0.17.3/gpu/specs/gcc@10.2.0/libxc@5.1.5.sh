...
spack spec --long --namespaces --types libxc@5.1.5 % gcc@10.2.0 +cuda cuda_arch=60,75,80,86 ~shared "^cuda@11.2.2/$(spack find --format '{hash:7}' cuda@11.2.2 % ${SPACK_COMPILER})"
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all libxc@5.1.5 % gcc@10.2.0 +cuda cuda_arch=60,75,80,86 ~shared "^cuda@11.2.2/$(spack find --format '{hash:7}' cuda@11.2.2 % ${SPACK_COMPILER})"
if [[ "${?}" -ne 0 ]]; then
