...
# [mkandes@login02 gpu]$ spack spec -l namd@2.14 % gcc@13.3.0 +cuda fftw=3 interface=tcl ~single_node_gpu cuda_arch=70,80,90 ^cuda@11.8.0
...
#          required because namd@2.14%gcc@13.3.0+cuda~single_node_gpu cuda_arch=70,80,90 fftw=3 interface=tcl ^cuda@11.8.0 requested explicitly 
...
declare -xr SPACK_VARIANTS='+cuda fftw=3 interface=tcl ~single_node_gpu cuda_arch=70,80,90'
declare -xr SPACK_DEPENDENCIES="^cuda@12.6.3/$(spack find --format '{hash:7}' cuda@12.6.3 % ${SPACK_COMPILER}) ^charmpp@6.10.2/$(spack find --format '{hash:7}' charmpp@6.10.2 % ${SPACK_COMPILER} +cuda backend=verbs ^cuda@12.6.3/$(spack find --format '{hash:7}' cuda@12.6.3 % ${SPACK_COMPILER})) ^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} ~mpi ~openmp)"
