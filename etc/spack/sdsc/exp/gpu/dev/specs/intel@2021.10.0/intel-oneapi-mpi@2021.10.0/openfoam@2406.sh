...
declare -xr SPACK_DEPENDENCIES="^adios2@2.10.1/$(spack find --format '{hash:7}' adios2@2.10.1 % ${SPACK_COMPILER} +mpi ^${SPACK_MPI}) ^cgal@5.6.1/$(spack find --format '{hash:7}' cgal@5.6.1 % ${SPACK_COMPILER}) ^intel-oneapi-mkl@2023.2.0/$(spack find --format '{hash:7}' intel-oneapi-mkl@2023.2.0 % ${SPACK_COMPILER} ~cluster ~ilp64 threads=none) ^kahip@3.16/$(spack find --format '{hash:7}' kahip@3.16 % ${SPACK_COMPILER} ^${SPACK_MPI}) ^scotch@7.0.5/$(spack find --format '{hash:7}' scotch@7.0.5 % ${SPACK_COMPILER} +mpi ^${SPACK_MPI}) ^zoltan@3.901/$(spack find --format '{hash:7}' zoltan@3.901 % ${SPACK_COMPILER} ~fortran +mpi +parmetis ^${SPACK_MPI})"
