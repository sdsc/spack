...
#SBATCH --job-name=gromacs@2024.3
...
declare -xr SPACK_PACKAGE='gromacs@2024.3'
declare -xr SPACK_COMPILER='gcc@13.3.0'
declare -xr SPACK_VARIANTS='~cp2k +cuda cuda_arch=70,80,90 ~cycle_subcounters ~double +hwloc ~intel_provided_gcc ~ipo ~mdrun_only +mpi ~nosuffix ~opencl +openmp +plumed ~relaxed_double_precision +shared ~sycl'
declare -xr SPACK_MPI='openmpi@4.1.6'
declare -xr SPACK_DEPENDENCIES="^openblas@0.3.28/$(spack find --format '{hash:7}' openblas@0.3.28 % ${SPACK_COMPILER} ~ilp64 threads=none) ^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} ~mpi ~openmp) ^${SPACK_MPI}/$(spack find --format '{hash:7}' ${SPACK_MPI} % ${SPACK_COMPILER}) ^plumed@2.9.3/$(spack find --format '{hash:7}' plumed@2.9.3 % ${SPACK_COMPILER} +mpi ^${SPACK_MPI}) ^cuda@12.6.3/$(spack find --format '{hash:7}' cuda@12.6.3 % ${SPACK_COMPILER})"
