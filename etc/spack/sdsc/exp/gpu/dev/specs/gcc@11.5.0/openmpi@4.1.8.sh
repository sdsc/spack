...
declare -xr SPACK_VARIANTS='~atomics +cuda cuda_arch=70,80 fabrics=ucx ~gpfs ~internal-hwloc ~internal-pmix +legacylaunchers +lustre ~memchecker ~openshmem ~orterunprefix +romio +rsh schedulers=slurm +static +vt'
declare -xr SPACK_DEPENDENCIES="^lustre ^slurm ^rdma-core ^ucx@1.14.1/$(spack find --format '{hash:7}' ucx@1.14.1 % ${SPACK_COMPILER}) ^cuda@11.8.0/$(spack find --format '{hash:7}' cuda@11.8.0 % ${SPACK_COMPILER})"
