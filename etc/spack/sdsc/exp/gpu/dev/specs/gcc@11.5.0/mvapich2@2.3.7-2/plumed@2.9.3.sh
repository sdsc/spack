...
declare -xr SPACK_VARIANTS='arrayfire=cuda +gsl +mpi optional_modules=all +shared'
declare -xr SPACK_MPI='mvapich2@2.3.7-2'
declare -xr SPACK_DEPENDENCIES="^openblas@0.3.28/$(spack find --format '{hash:7}' openblas@0.3.28 % ${SPACK_COMPILER} ~ilp64 threads=none) ^${SPACK_MPI}/$(spack find --format '{hash:7}' ${SPACK_MPI} % ${SPACK_COMPILER}) ^arrayfire@3.9.0/$(spack find --format '{hash:7}' arrayfire@3.9.0 % ${SPACK_COMPILER})" 
