...
declare -xr SPACK_VARIANTS='+atomic +chrono ~clanglibcpp +container +context +coroutine +date_time ~debug +exception +fiber +filesystem +graph +graph_parallel ~icu +iostreams +json +locale +log +math ~mpi +multithreaded ~nowide +numpy +pic +program_options +python +random +regex +serialization +shared ~signals ~singlethreaded +stacktrace +system ~taggedlayout +test +thread +timer +type_erasure ~versionedlayout +wave cxxstd=11'
declare -xr SPACK_DEPENDENCIES="^py-numpy@1.26.4/$(spack find --format '{hash:7}' py-numpy@1.26.4 % ${SPACK_COMPILER} ^intel-oneapi-mkl@2023.2.0/$(spack find --format '{hash:7}' intel-oneapi-mkl@2023.2.0 % ${SPACK_COMPILER} ~cluster ~ilp64 threads=none))"
