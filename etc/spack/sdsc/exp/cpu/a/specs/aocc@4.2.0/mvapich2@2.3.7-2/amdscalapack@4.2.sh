...
#SBATCH --job-name=amdscalapack@4.2
...
declare -xr SPACK_PACKAGE='amdscalapack@4.2'
declare -xr SPACK_COMPILER='aocc@4.2.0'
declare -xr SPACK_VARIANTS='~ilp64 ~ipo +pic +shared'
declare -xr SPACK_MPI='mvapich2@2.3.7-2'
declare -xr SPACK_DEPENDENCIES="^amdblis@4.2/$(spack find --format '{hash:7}' amdblis@4.2 % ${SPACK_COMPILER} ~ilp64 threads=none) ^amdlibflame@4.2/$(spack find --format '{hash:7}' amdlibflame@4.2 % ${SPACK_COMPILER} ~ilp64 ^amdblis@4.2 threads=none) ^${SPACK_MPI}/$(spack find --format '{hash:7}' ${SPACK_MPI} % ${SPACK_COMPILER})"
