...
declare -xr SPACK_VARIANTS='~cp2k ~cuda ~cycle_subcounters ~double +hwloc ~intel_provided_gcc ~ipo ~mdrun_only +mpi ~nosuffix ~opencl +openmp ~relaxed_double_precision +shared ~sycl'
declare -xr SPACK_MPI='openmpi@4.1.6'
declare -xr SPACK_DEPENDENCIES="^openblas@0.3.28/$(spack find --format '{hash:7}' openblas@0.3.28 % ${SPACK_COMPILER} ~ilp64 threads=none) ^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} ~mpi ~openmp) ^${SPACK_MPI}/$(spack find --format '{hash:7}' ${SPACK_MPI} % ${SPACK_COMPILER})" 
