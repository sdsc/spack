...
declare -xr SPACK_PACKAGE='neuron@8.2.3'
declare -xr SPACK_COMPILER='aocc@4.2.0'
declare -xr SPACK_VARIANTS='~backtrace ~caliper +coreneuron ~interviews ~legacy-unit +mpi +python +rx3d +shared ~tests'
declare -xr SPACK_MPI='mvapich2@2.3.7-2'
declare -xr SPACK_DEPENDENCIES="^${SPACK_MPI}/$(spack find --format '{hash:7}' ${SPACK_MPI} % ${SPACK_COMPILER}) ^py-numpy@2.1.2/$(spack find --format '{hash:7}' py-numpy@2.1.2 % ${SPACK_COMPILER} ^amdblis@4.2/$(spack find --format '{hash:7}' amdblis@4.2 % ${SPACK_COMPILER} ~ilp64 threads=none) ^amdlibflame@4.2/$(spack find --format '{hash:7}' amdlibflame@4.2 % ${SPACK_COMPILER} ~ilp64 ^amdblis@4.2 threads=none))"
