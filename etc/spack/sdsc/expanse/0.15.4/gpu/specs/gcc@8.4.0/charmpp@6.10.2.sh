...
spack spec --long --namespaces --types charmpp@6.10.2 % gcc@8.4.0 backend=verbs build-target=charm++ ~cuda +omp ~papi pmi=none +production ~pthreads +shared +smp ~syncft ~tcp ~tracing
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all charmpp@6.10.2 % gcc@8.4.0 backend=verbs build-target=charm++ ~cuda +omp ~papi pmi=none +production ~pthreads +shared +smp ~syncft ~tcp ~tracing
if [[ "${?}" -ne 0 ]]; then
