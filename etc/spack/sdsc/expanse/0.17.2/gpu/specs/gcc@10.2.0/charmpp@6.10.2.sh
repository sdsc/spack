...
#SBATCH --job-name=charmpp@6.10.2-verbs
...
spack spec --long --namespaces --types charmpp@6.10.2 % gcc@10.2.0 backend=verbs build-target=charm++ +cuda ~omp ~papi pmi=none +production ~pthreads +shared +smp ~syncft ~tcp ~tracing "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all charmpp@6.10.2 % gcc@10.2.0 backend=verbs build-target=charm++ +cuda ~omp ~papi pmi=none +production ~pthreads +shared +smp ~syncft ~tcp ~tracing "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
