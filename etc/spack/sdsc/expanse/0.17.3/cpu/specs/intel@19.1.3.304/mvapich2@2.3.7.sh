...
spack spec --long --namespaces --types mvapich2@2.3.7 % intel@19.1.3.304 ~alloca ch3_rank_bits=32 ~cuda ~debug file_systems=lustre process_managers=slurm +regcache threads=multiple +wrapperrpath ^slurm@21.08.8 ^rdma-core@43.0
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all mvapich2@2.3.7 % intel@19.1.3.304 ~alloca ch3_rank_bits=32 ~cuda ~debug file_systems=lustre process_managers=slurm +regcache threads=multiple +wrapperrpath ^slurm@21.08.8 ^rdma-core@43.0
if [[ "${?}" -ne 0 ]]; then
