...
#SBATCH --job-name=gamess@2021.09
...
declare -xr SPACK_VARIANTS='~libxc mathlib=openblas'
...
time -p spack spec --long --namespaces --types gamess@2021.09 % gcc@10.2.0 ~libxc mathlib=openblas "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all gamess@2021.09 % gcc@10.2.0 ~libxc mathlib=openblas "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
