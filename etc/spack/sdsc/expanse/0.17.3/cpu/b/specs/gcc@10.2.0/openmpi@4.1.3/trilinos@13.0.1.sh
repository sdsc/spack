...
# [mkandes@login02 spack]$ time -p spack spec -l trilinos@13.0.1 % gcc@10.2.0 +adios2 +amesos +amesos2 +anasazi +aztec ~basker +belos +boost ~chaco ~complex ~cuda ~cuda_rdc ~debug ~dtk +epetra +epetraext ~epetraextbtf ~epetraextexperimental ~epetraextgraphreorderings ~exodus +explicit_template_instantiation ~float +fortran +gtest +hdf5 +hypre +ifpack +ifpack2 ~intrepid ~intrepid2 ~ipo ~isorropia +kokkos ~mesquite ~minitensor +ml +mpi +muelu +mumps ~nox ~openmp ~phalanx ~piro +python ~rol ~rythmos +sacado ~scorec ~shards +shared ~shylu ~stk ~stokhos ~stratimikos ~strumpack +suite-sparse +superlu +superlu-dist ~teko ~tempus +tpetra ~trilinoscouplings ~wrapper ~x11 +zoltan +zoltan2
#Input spec
#--------------------------------
