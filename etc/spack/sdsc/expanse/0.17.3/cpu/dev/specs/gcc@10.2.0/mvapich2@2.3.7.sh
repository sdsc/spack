...
time -p spack spec --long --namespaces --types --reuse mvapich2@2.3.7 % "${SPACK_COMPILER}" ~alloca ch3_rank_bits='32' ~cuda ~debug file_systems='nfs,lustre' process_managers='slurm' +regcache threads='multiple' +wrapperrpath "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse mvapich2@2.3.7 % "${SPACK_COMPILER}" ~alloca ch3_rank_bits='32' ~cuda ~debug file_systems='nfs,lustre' process_managers='slurm' +regcache threads='multiple' +wrapperrpath "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
