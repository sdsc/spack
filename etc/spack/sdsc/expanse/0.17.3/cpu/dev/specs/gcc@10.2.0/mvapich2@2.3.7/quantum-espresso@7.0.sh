...
declare -xr SPACK_VARIANTS='+cmake +elpa ~environ +epw ~ipo +libxc +mpi ~openmp +patch +qmcpack +scalapack'
declare -xr SPACK_DEPENDENCIES="^openblas@0.3.18/$(spack find --format '{hash:7}' openblas@0.3.18 % ${SPACK_COMPILER} ~ilp64 threads=none) ^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} ~mpi ~openmp) ^python@3.8.12/$(spack find --format '{hash:7}' python@3.8.12 % ${SPACK_COMPILER}) ^mvapich2@2.3.7/$(spack find --format '{hash:7}' mvapich2@2.3.7 % ${SPACK_COMPILER}) ^netlib-scalapack@2.1.0/$(spack find --format '{hash:7}' netlib-scalapack@2.1.0 % ${SPACK_COMPILER} ^openblas@0.3.18 ^mvapich2@2.3.7) ^elpa@2021.05.001/$(spack find --format '{hash:7}' elpa@2021.05.001 % ${SPACK_COMPILER} +mpi ~openmp ^mvapich2@2.3.7)"
