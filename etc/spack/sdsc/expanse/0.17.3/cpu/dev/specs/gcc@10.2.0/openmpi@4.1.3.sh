...
time -p spack spec --long --namespaces --types --reuse openmpi@4.1.3 % "${SPACK_COMPILER}" ~atomics~cuda~cxx~cxx_exceptions~gpfs~internal-hwloc~java+legacylaunchers+lustre~memchecker+pmi+pmix+romio~rsh~singularity+static+vt+wrapper-rpath cuda_arch='none' fabrics='ucx' schedulers='slurm' "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
