...
time -p spack spec --long --namespaces --types --reuse wps@4.2 % gcc@10.2.0 build_type='dmpar' "^wrf@4.2/$(spack find --format '{hash:7}' wrf@4.2 % ${SPACK_COMPILER} ^openmpi@4.1.3)" 
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse wps@4.2 % gcc@10.2.0 build_type='dmpar' "^wrf@4.2/$(spack find --format '{hash:7}' wrf@4.2 % ${SPACK_COMPILER} ^openmpi@4.1.3)"
if [[ "${?}" -ne 0 ]]; then
