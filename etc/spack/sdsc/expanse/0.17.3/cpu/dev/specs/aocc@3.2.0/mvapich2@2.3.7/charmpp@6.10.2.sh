...
time -p spack spec --long --namespaces --types --reuse charmpp@6.10.2 % aocc@3.2.0 backend=mpi build-target=charm++ ~cuda +omp ~papi pmi=none +production ~pthreads +shared +smp ~syncft ~tcp ~tracing "^mvapich2@2.3.7/$(spack find --format '{hash:7}' mvapich2@2.3.7 % ${SPACK_COMPILER})"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse charmpp@6.10.2 % aocc@3.2.0 backend=mpi build-target=charm++ ~cuda +omp ~papi pmi=none +production ~pthreads +shared +smp ~syncft ~tcp ~tracing "^mvapich2@2.3.7/$(spack find --format '{hash:7}' mvapich2@2.3.7 % ${SPACK_COMPILER})"
if [[ "${?}" -ne 0 ]]; then
