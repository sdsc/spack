...
time -p spack spec --long --namespaces --types --reuse quantum-espresso@7.0 % aocc@3.2.0 +cmake ~elpa ~environ +epw ~ipo hdf5=parallel +libxc +mpi ~openmp +patch +qmcpack +scalapack "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
