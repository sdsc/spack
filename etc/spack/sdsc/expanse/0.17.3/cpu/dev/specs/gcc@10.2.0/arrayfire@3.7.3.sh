...
declare -xr SPACK_VARIANTS='~cuda cuda_arch=none ~forge ~ipo ~opencl'
declare -xr SPACK_DEPENDENCIES="^openblas@0.3.18/$(spack find --format '{hash:7}' openblas@0.3.18 % ${SPACK_COMPILER} ~ilp64 threads=none) ^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} ~mpi ~openmp) ^boost@1.77.0/$(spack find --format '{hash:7}' boost@1.77.0 % ${SPACK_COMPILER} ~mpi)"
...
time -p spack spec --long --namespaces --types --reuse arrayfire@3.7.3 % "${SPACK_COMPILER}" ~cuda cuda_arch='none' ~forge ~ipo ~opencl "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse arrayfire@3.7.3 % "${SPACK_COMPILER}" ~cuda cuda_arch='none' ~forge ~ipo ~opencl "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
