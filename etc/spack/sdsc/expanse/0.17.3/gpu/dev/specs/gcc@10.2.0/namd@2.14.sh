...
declare -xr SPACK_VARIANTS='+cuda cuda_arch=70,80 fftw=3 interface=tcl'
declare -xr SPACK_DEPENDENCIES="^charmpp@6.10.2/$(spack find --format '{hash:7}' charmpp@6.10.2 % ${SPACK_COMPILER} backend=multicore) ^fftw@3.3.10/$(spack find --format '{hash:7}' fftw@3.3.10 % ${SPACK_COMPILER} ~mpi ~openmp)"
...
time -p spack spec --long --namespaces --types --reuse namd@2.14 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' fftw='3' interface='tcl' "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse namd@2.14 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' fftw='3' interface='tcl' "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
