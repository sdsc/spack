...
time -p spack spec --long --namespaces --types --reuse arrayfire@3.7.3 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' ~forge ~ipo ~opencl "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse arrayfire@3.7.3 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' ~forge ~ipo ~opencl "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
