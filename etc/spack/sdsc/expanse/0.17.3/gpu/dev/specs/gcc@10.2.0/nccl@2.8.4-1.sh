...
time -p spack spec --long --namespaces --types --reuse nccl@2.8.4-1 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse nccl@2.8.4-1 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' "${SPACK_DEPENDENCIES}"
