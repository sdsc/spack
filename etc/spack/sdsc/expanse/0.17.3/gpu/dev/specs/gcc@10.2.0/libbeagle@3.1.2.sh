...
time -p spack spec --long --namespaces --types --reuse libbeagle@3.1.2 % "${SPACK_COMPILER}" +cuda cuda_arch='70' "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse libbeagle@3.1.2 % "${SPACK_COMPILER}" +cuda cuda_arch='70' "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
