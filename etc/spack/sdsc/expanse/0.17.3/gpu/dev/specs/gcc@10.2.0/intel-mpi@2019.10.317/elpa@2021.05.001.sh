...
declare -xr SPACK_VARIANTS='+cuda cuda_arch=70,80 +mpi ~openmp ~rocm'
declare -xr SPACK_DEPENDENCIES="^cuda@11.2.2/$(spack find --format '{hash:7}' cuda@11.2.2 % ${SPACK_COMPILER}) ^openblas@0.3.18/$(spack find --format '{hash:7}' openblas@0.3.18 % ${SPACK_COMPILER} ~ilp64 threads=none) ^python@3.8.12/$(spack find --format '{hash:7}' python@3.8.12 % ${SPACK_COMPILER}) ^intel-mpi@2019.10.317/$(spack find --format '{hash:7}' intel-mpi@2019.10.317 % ${SPACK_COMPILER}) ^netlib-scalapack@2.1.0/$(spack find --format '{hash:7}' netlib-scalapack@2.1.0 % ${SPACK_COMPILER} ^openblas@0.3.18 ^intel-mpi@2019.10.317)"
...
time -p spack spec --long --namespaces --types --reuse elpa@2021.05.001 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' +mpi ~openmp ~rocm "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse elpa@2021.05.001 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' +mpi ~openmp ~rocm "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
