...
time -p spack spec --long --namespaces --types --reuse libxc@5.1.5 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' ~shared "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse libxc@5.1.5 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' ~shared "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
