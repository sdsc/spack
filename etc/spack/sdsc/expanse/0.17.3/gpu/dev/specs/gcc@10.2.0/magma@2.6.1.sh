...
time -p spack spec --long --namespaces --types --reuse magma@2.6.1 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' +fortran ~ipo ~rocm +shared "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
...
time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all --reuse magma@2.6.1 % "${SPACK_COMPILER}" +cuda cuda_arch='70,80' +fortran ~ipo ~rocm +shared "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
