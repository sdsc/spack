...
#SBATCH --job-name=lammps@20230802
...
#  version_satisfies("lammps","20181212:","20230802")
#  version_satisfies("lammps","20230802")
...
#            ack-stage/spack-stage-lammps-20230802-il34zpxttkmdye5sk2shvxlxolpi6
#            tux/spack-src/cmake
#     337        linked by target "gpu" in directory /tmp/mkandes/spack-stage/sp
#            ack-stage-lammps-20230802-il34zpxttkmdye5sk2shvxlxolpi6tux/spack-sr
#            c/cmake
#     338    
#     339    -- Generating done
...
#     61    
#     62        for file: [/tmp/mkandes/spack-stage/spack-stage-lammps-20230802-
...
#     74    
#     75        for file: [/tmp/mkandes/spack-stage/spack-stage-lammps-20230802-
...
#  condition(3088)
#  condition(3089)
#  conflict("lammps",3088,3089)
#  no version satisfies the given constraints
#  node("openblas")
#  variant_condition(2816,"lammps","ml-iap")
#  variant_condition(2819,"lammps","ml-snap")
#  variant_set("lammps","ml-iap","True")
#  variant_set("lammps","ml-snap","False")
#  version_satisfies("lammps","20210702:","20230802")
#  version_satisfies("lammps","20230802")

declare -xr SPACK_PACKAGE='lammps@20230802'
declare -xr SPACK_COMPILER='gcc@10.2.0'
declare -xr SPACK_VARIANTS='~adios amdgpu_target=none ~amoeba +asphere +atc +awpmd +bocs +body ~bpm ~brownian build_type=RelWithDebInfo ~cg-dna ~cg-sdk ~cg-spica +class2 +colloid +colvars +compress +coreshell +cuda cuda_arch=70 ~cuda_mps ~dielectric +diffraction +dipole +dpd-basic +dpd-meso +dpd-react +dpd-smooth +drude +eff ~electrode ~exceptions ~extra-compute ~extra-dump ~extra-fix ~extra-molecule ~extra-pair +fep ~ffmpeg fftw_precision=double gpu_precision=mixed +granular ~h5md ~intel ~interlayer ~ipo +jpeg +kim +kokkos +kspace lammps_sizes=smallbig ~latboltz ~latte ~lepton +lib ~machdyn +manifold +manybody +mc ~meam +mesont +mgpt +misc ~ml-hdnnp ~ml-iap ~ml-pod ~ml-rann ~ml-snap ~mliap +mofff +molecule ~molfile +mpi +mpiio ~netcdf ~opencl +openmp +openmp-package +opt ~orient +peri +phonon ~plugin ~plumed +png +poems +ptm +python +qeq +qtb +reaction ~reax +reaxff +replica +rigid ~rocm +shock +smtbq ~snap +sph +spin +srd +tally +uef ~user-adios ~user-atc ~user-awpmd ~user-bocs ~user-brownian ~user-cgsdk ~user-colvars ~user-diffraction ~user-dpd ~user-drude ~user-eff ~user-fep ~user-h5md ~user-hdnnp ~user-intel ~user-lb ~user-manifold ~user-meamc ~user-mesodpd ~user-mesont ~user-mgpt ~user-misc ~user-mofff ~user-molfile ~user-netcdf ~user-omp ~user-phonon ~user-plumed ~user-ptm ~user-qtb ~user-rann ~user-reaction ~user-reaxc ~user-sdpd ~user-smd ~user-smtbq ~user-sph ~user-tally ~user-uef ~user-vtk ~user-yaff ~voronoi ~vtk +yaff'
