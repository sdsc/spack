...
time -p spack spec --long --namespaces --types amber@20 % gcc@10.2.0 +cuda cuda_arch=70,80 +mpi +openmp +update "${SPACK_DEPENDENCIES}"
if [[ "${?}" -ne 0 ]]; then
  echo 'ERROR: spack concretization failed.'
  exit 1
fi

time -p spack install --jobs "${SLURM_CPUS_PER_TASK}" --fail-fast --yes-to-all amber@20 % gcc@10.2.0 +cuda cuda_arch=70,80 +mpi +openmp +update "${SPACK_DEPENDENCIES}"
sleep 3600
if [[ "${?}" -ne 0 ]]; then
