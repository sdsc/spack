...
class GmapGsnap(AutotoolsPackage):
    ...
    def install_test(self):
        mkdirp(prefix.tests)
        install(join_path('tests','ss.chr17test'),prefix.tests)
        install(join_path('tests','ss.her2'),prefix.tests)
        with working_dir(join_path(self.prefix.tests)):
            output=Executable(join_path(prefix.bin,'gmap'))('-A','-g',join_path(self.prefix.tests,'ss.chr17test'),join_path(self.prefix.tests,'ss.her2'),output=str,error=str)
            teststring='Trimmed coverage: 100.0 (trimmed length: 4624 bp, trimmed region: 1..4624)'
            with open('/tmp/gmap-gsnap.output','w') as fp:
