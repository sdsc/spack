...
class Geos(CMakePackage):
    ...
    def install_test(self):
        mkdirp(prefix.test)
        install(join_path(os.path.dirname(__file__),'geos.c'),prefix.test)
        with working_dir(self.prefix.test):
             output=Executable(spack_cxx)('-I',self.prefix.include,'-L',self.prefix.lib,'-lgeos','-o','geos.exe','geos.c',output=str,error=str)
             output=Executable('./geos.exe')(output=str,error=str)
             teststring='ported from'
             with open('/tmp/geos.output','w') as fp:
