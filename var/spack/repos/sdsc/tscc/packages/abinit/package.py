...
class Abinit(AutotoolsPackage):
    ...
    def setup_build_environment(self, env):
        env.set('I_MPI_PMI_LIBRARY','/cm/shared/apps/slurm/current/lib64/libpmi2.so')
        env.set('ABI_PSPDIR',join_path(self.prefix.tests,'Psps_for_tests'))

    @run_after('install')

    def test(self):
        if '@9.6.1:' in self.spec:
            install_tree('tests',self.spec.prefix.tests)
            psp_path=join_path(self.spec.prefix.tests,'Psps_for_tests','6c_lda.paw')
            with working_dir(join_path(self.spec.prefix.tests,'paral','Input')):
               output=Executable('srun')('--mpi=pmi2',join_path(self.spec.prefix.bin,'abinit'),'t01.abi',output=str,error=str)
               teststring='energy (hartree) =   0.26186'
