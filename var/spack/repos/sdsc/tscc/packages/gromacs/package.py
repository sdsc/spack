...
#           Executable('tar')('xf',join_path(os.path.dirname(self.module.__file__),'gmxtest.tgz'))
#           with working_dir(join_path('gmxtest','complex','acetonitrilRF')):
#               Executable('srun')('-n','1','--mpi=pmi2',join_path(self.prefix.bin,'gmx_mpi'),'grompp',output=str,error=str)
#               Executable('srun')('-n','4','--ntasks-per-node=4','--mpi=pmi2',join_path(self.prefix.bin,'gmx_mpi'),'mdrun',output=str,error=str)
#               output=Executable('cat')('md.log',output=str,error=str)
#               teststring='-9.24502e+03   -1.30228e+03'
#               with open('tmp/gromacs.output','w') as fp:
