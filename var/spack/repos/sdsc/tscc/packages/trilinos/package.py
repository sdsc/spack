...
class Trilinos(CMakePackage, CudaPackage):
    ...
    def cmake_args(self):
        ...
        if "gfortran" in self.compiler.fc:
            options.extend([
                define("CMAKE_EXE_LINKER_FLAGS:STRING","-lgfortran")
            ])
    ...
    def test(self):
        ...
        with working_dir(self.prefix.test):
            copy(join_path(os.path.dirname(self.module.__file__),'test.cxx'),'.')
            output=Executable('mpicxx')('-I../include','-o','test','test.cxx','-L../lib','-lteuchoscore',output=str)
            output=Executable('./test')(output=str)
            teststring='Teuchos in Trilinos'
            with open('/tmp/trilinos.output','w') as fp:
