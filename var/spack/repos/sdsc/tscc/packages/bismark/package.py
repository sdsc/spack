...
class Bismark(Package):
    ...
    def setup_build_environment(self, env):
        env.prepend_path('PATH',join_path(self.spec['bowtie2'].prefix,'bin'))
    ...
    def test(self):
        output=Executable(join_path(prefix.bin,'bismark'))(output=str,error=str)
        teststring='USAGE: bismark [options] <genome_folder> {-1 <mates1> -2 <mates2> | <singles>}'
        with open('/tmp/bismark.output','w') as fp:
