...
class Proj(AutotoolsPackage):
    ...
    def test(self):
        ...
        with working_dir(self.prefix.test):
            copy(join_path(os.path.dirname(self.module.__file__),'input'),'.')
            with open('input',mode='r') as fp:
                output=Executable(join_path(self.prefix.bin,'proj'))('+proj=poly','+ellps=clrk66',input=fp,output=str)
            teststring='786491.58	5033320.60'
            with open('/tmp/proj.output','w') as fp:
