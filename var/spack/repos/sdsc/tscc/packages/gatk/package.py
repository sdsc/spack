...
class Gatk(Package):
    ...
    def test(self):
        mkdirp(join_path(prefix,'resources'))
        install_tree(join_path(os.path.dirname(self.module.__file__),'resources'),join_path(self.prefix,'resources'))
        with working_dir(join_path(self.prefix,'resources')):
            output=Executable(join_path(self.prefix.bin,'gatk'))('CountReads','-R','exampleFASTA.fasta','-I','exampleBAM.bam',output=str,error=str)
            teststring='ProgressMeter - Traversal complete. Processed 33 total reads'
            with open('/tmp/gatk.output','w') as fp:
