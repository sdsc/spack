...
class Soapdenovo2(MakefilePackage):
    ...
    def test(self):
        output=Executable(join_path(prefix.bin,'SOAPdenovo-63mer'))(output=str,error=str)
        teststring='Usage: SOAPdenovo <command> [option]'
        with open('/tmp/soapdenovo2.output','w') as fp:
