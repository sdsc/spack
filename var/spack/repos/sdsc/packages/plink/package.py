...
class Plink(Package):
    ...
    def install_test(self):
        with working_dir(self.prefix.examples):
            output=Executable('plink')('--noweb','--file','hapmap1',output=str,error=str)
            output=Executable('cat')('plink.log',output=str,error=str)
            teststring='After filtering, 44 cases, 45 controls and 0 missing'
            with open('/tmp/plink.output','w') as fp:
