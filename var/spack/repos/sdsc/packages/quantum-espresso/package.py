...
class QuantumEspresso(Package):
    ...
    url = 'https://github.com/QEF/q-e/archive/qe-6.6.tar.gz'
    git = 'https://gitlab.com/QEF/q-e.git'

    maintainers = ['naromero77']

    version('develop', branch='develop')
    version('6.7.0', sha256='11364f931894555f296f176ca91757f9a8d510f8e5264ced60e82e11c6de725f')
    ...
    # Enables Environ patch
    variant('environ', default=False, description='Builds environ version')
    ...
    def install(self, spec, prefix):
        ...
        if '+epw' in spec:
            ...
            if '+environ' in spec:
                make('pw')
                subprocess.run(['/usr/bin/git','clone','https://gitlab.com/olivieroandreussi/Environ.git'])
                subprocess.run(['./install/addsonpatch.sh','Environ','Environ/src','Modules','-patch'])
                subprocess.run(['./Environ/patches/environpatch.sh','-patch'])
                subprocess.run(['./install/makedeps.sh'])
                make('pw')
            else:
                make('all')
