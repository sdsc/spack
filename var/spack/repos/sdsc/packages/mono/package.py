...
class Mono(AutotoolsPackage):
    ...
    def test(self):
        ...
        with working_dir(prefix.test):
           copy(join_path(os.path.dirname(self.module.__file__),'test.cs'),'.')
           Executable(join_path(prefix.bin,'mcs'))('-o','test','test.cs')
           output=Executable(join_path(prefix.bin,'mono'))('test',output=str,error=str)
           teststring='Hello world'
           with open('/tmp/mono.output','w') as fp:
