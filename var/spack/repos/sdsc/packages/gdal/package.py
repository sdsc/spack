...
class Gdal(AutotoolsPackage):
    ...
    def install_test(self):
        mkdirp(self.prefix.test)
        install(join_path(os.path.dirname(__file__),'gdal.c'),self.prefix.test)
        with working_dir(self.prefix.test):
             output=Executable(spack_cc)('-I',self.prefix.include,'-L',self.prefix.lib,'-lgdal','-o','gdal.exe','gdal.c',output=str,error=str)      
             output=Executable('./gdal.exe')(output=str,error=str)
             teststring='worked'
             with open('/tmp/gdal.output','w') as fp:
