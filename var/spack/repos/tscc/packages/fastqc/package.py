...
class Fastqc(Package):
    ...
    def test(self):
        output=Executable(join_path(self.prefix.bin,'fastqc'))('--help',output=str,error=str)
        teststring='FastQC - A high throughput sequence QC analysis tool'
        with open('/tmp/fastqc.output','w') as fp:
